...
var (
	s1v = &s1{
		Name:     "Bob Smith",
		BirthDay: JSONTime{time.Date(2013, 1, 2, 3, 4, 5, 0, time.UTC)},
		Phone:    "5551234567",
		Siblings: 2,
		Spouse:   false,
		Money:    100.0,
	...
	svb = []byte{0x9, 0x42, 0x6f, 0x62, 0x20, 0x53, 0x6d, 0x69, 0x74,
		0x68, 0xa5, 0xa3, 0xe3, 0x50, 0x0a, 0x35, 0x35, 0x35, 0x31,
		0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x2, 0x0, 0x0, 0x0, 0x0,
...
func TestBinaryMarshalUnMarshaler(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, []byte{0x13}, b) // bytes don't implement varint natively.. see HexBytes
}

func TestHexBytesMarshalUnMarshaler(t *testing.T) {
	s2v := HexBytes([]byte{0x13})
	b, err := MarshalBinary(s2v)
	assert.NoError(t, err)
	assert.Equal(t, []byte{0x01, 0x13}, b) // HexBytes does varint prefixing.. and hex serialization in JSON
}
